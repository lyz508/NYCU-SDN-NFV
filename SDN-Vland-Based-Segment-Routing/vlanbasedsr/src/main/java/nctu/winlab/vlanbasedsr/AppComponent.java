...
public class AppComponent{
    ...
    private VlanBasedSRProcessor vlanbasedsrProcessor  = new VlanBasedSRProcessor();
    ...
    protected void deactivate() {
        ...
        packetService.removeProcessor(vlanbasedsrProcessor);
            ...
            for (int j=0; j<edgeSwitchList.size(); j++){
                if (i != j){
                    String subnets = edgeSwitchSubnetMap.get(edgeSwitchList.get(j)),
                            ...
                            edgeSwitchPortNumber = edgeSwitchPortMap.get(edgeSwitchList.get(i));
                    log.info("ip {}, mask {}", subnetIP, subnetMask);
                    int subnetPrefix = Integer.parseInt(subnetMask);
                    Long outputToVlanNetwork = Long.valueOf(Integer.parseInt(edgeSwitchPortNumber));
                    ...
                    log.info("Port Number: {}", outputToVlanNetwork);
                    PortNumber outputTarget = PortNumber.portNumber(outputToVlanNetwork);

                    IpPrefix testPrefix = IpPrefix.valueOf(IpAddress.valueOf("10.0.0.0"), 24);
                    ...
                    //         DefaultTrafficSelector.builder()
                    //         .matchEthType(Ethernet.TYPE_IPV4)
                    //         .matchIPDst(IpPrefix.valueOf(IpAddress.valueOf("10.0.0.0"), 16))
                    //         .build()
                    //     )
                    //     .withTreatment(
                    //         DefaultTrafficTreatment.builder()
    ...
    private class VlanBasedSRProcessor implements PacketProcessor {
        ...
        public void process (PacketContext pc){
            ...
            Boolean isToEdge = false;
            short tmpId;

            if (etherFrame.getEtherType() != Ethernet.TYPE_IPV4){
                // log.info("a");
                // log.info("{}", etherFrame.getEtherType());
                // log.info("{}", Ethernet.TYPE_IPV4);
                return;
            }
            else{
                tmpId = etherFrame.getVlanID();
                log.info("Vlan ID: {}", tmpId);
            }
            if (etherFrame.getDestinationMAC().isLldp()){
                log.info("b");
                return;
            }
            if (edgeSwitchList.size() <= 0){
                log.info("Config haven't be provided!");
                return;
            }

            // VlanId targetId = VlanId.vlanId(tmpId); // target Vlan ID
            // DeviceId targetDevice = DeviceId.deviceId("of:0000000000000001"); // target edge switch (with default value)

            // // Judge, find the edge switch corresponding to the vlanId and build the path
            // for (int i=0; i<edgeSwitchList.size(); i++){
            //     VlanId dstVlanId = VlanId.vlanId(edgeSwitchIDMap.get(edgeSwitchList.get(i)));
            //     if (dstVlanId.equals(targetId)){
            //         targetDevice = edgeSwitchList.get(i);
            //         log.info("Segment ID match, Vlan ID {} match to {}", dstVlanId.toString(), targetDevice.toString());
            //         isToEdge = true;
            //         break;
            //     }
            // }

            // // Calculate & apply flow rule
            // if (isToEdge){
            //     Set<Path> res;
            //     res = pathService.getPaths(cp.deviceId(), targetDevice);
            //     log.info("From Device {} to {}", cp.deviceId().toString(), targetDevice.toString());
            ...
            //             // Build Flow Rule to construct the path
            //             installFlowRule(pc, lks, targetId);
            //         }
            //     }
            //     else{
            //         return;
            ...
            if ((event.type() == CONFIG_ADDED || event.type() == CONFIG_UPDATED)
                ...
                if (config != null) {
                    JsonNode configNodes = config.node(),
                            ...
                            edgeSwitchPortNode = configNodes.findValue("edgeSwitchPort");
                    ...
                    // Map Edge Switch and port to Vlan network
                    eles = edgeSwitchPortNode.elements();
                    for (int i=0; i<edgeSwitchList.size(); i++){
                        JsonNode now = eles.next();
                        edgeSwitchPortMap.put(edgeSwitchList.get(i), now.textValue());
                    }
