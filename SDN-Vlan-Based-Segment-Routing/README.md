# SDN Vlan-Based Segment Routing
##### Final project of NYCU SDN-NFV

## Lib
- TrafficTreatment
    - for push/pop Vlan tag
- TrafficSelector
    - for choosing for indicatenetmask and IP destination
    - include 'org.onlab.packet.IpPrefix' -> match IP prefix
- FlowObjectiveService
    - Forwarding traffic


## Tech
- Generate IP Prefix with IP Address
```java=
// Building IP Prefix
IpAddress test = IpAddress.valueOf("10.0.2.0");
IpPrefix ipPrefix = IpPrefix.valueOf(test, 24);

// Testing IP Address with IP Prefix
log.info("Test for 10.0.2.12: {}", ipPrefix.contains(IpAddress.valueOf("10.0.2.12")));
log.info("Test for 10.0.3.12: {}", ipPrefix.contains(IpAddress.valueOf("10.0.3.12")));
```

- Observe Vlan Packet
```java=
sudo tcpdump -nvi <NIC> -e vlan
```
